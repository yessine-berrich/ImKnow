...
export default function ReportedUsersPage() {
  ...
  const handleExport = (format: 'csv' | 'json' | 'pdf') => {
    ...
    if (format === 'json') {
      ...
      filename = 'signalements-utilisateurs.json';
    } else {
      const esc = (s: string) => `"${s.replace(/"/g, '""')}"`;
      const header = ['ID', 'Nom', 'Email', 'Département', 'Actif', 'Niveau de risque', 'Score', 'Signalements', 'En attente', 'Motif principal', 'Tendance', 'Dernier signalement'];
      ...
      filename = 'signalements-utilisateurs.csv';
    }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Utilisateurs signalés</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {data?.total ?? 0} utilisateur(s) signalé(s) · {summary?.bannedUsers ?? 0} suspendu(s)
            </p>
          </div>
        ...
        {/* Summary Cards — clickable pour filtrer */}
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {([
            { label: 'Urgent',    value: summary?.urgent ?? 0,       color: 'text-red-600 dark:text-red-400',      bg: 'bg-red-50 dark:bg-red-900/20',      ring: 'ring-red-400',    isActive: priorityFilter === 'urgent',   click: () => { setPriorityFilter(priorityFilter === 'urgent' ? 'all' : 'urgent'); setPage(1); } },
