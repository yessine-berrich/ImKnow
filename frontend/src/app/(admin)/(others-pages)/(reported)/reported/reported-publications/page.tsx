...
import {
  ...
  PublicationReportListResponse, RiskLevel, PublicationAction,
...
const PUBLICATION_STATUS_LABELS: Record<string, { label: string; cls: string }> = {
...
function PublicationDetailDrawer({
  ...
  const handleAction = async (action: PublicationAction) => {
    ...
    setConfirm(null);
    try {
      const res = await adminReportsService.takePublicationAction(publicationId, action, note || undefined);
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      {confirm && (
        <ConfirmModal
          title={confirm.action === 'unpublish' ? 'Dépublier cet publication ?' : 'Republier cet publication ?'}
          description={
            confirm.action === 'unpublish'
              ? "L'publication sera masqué et l'auteur sera notifié. Cette action est réversible."
              : "L'publication sera rendu public à nouveau et les signalements seront clôturés."
          }
      ...
      <div className="w-full max-w-xl bg-white dark:bg-gray-900 h-full overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold text-gray-900 dark:text-white">Détail — Publication signalé</h2>
          </div>
          ...
          <div className="flex-1 p-6 space-y-6">
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-800/60 rounded-xl border border-gray-200 dark:border-gray-700 space-y-2">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-semibold flex-shrink-0 ${PUBLICATION_STATUS_LABELS[detail.publication.status]?.cls ?? 'text-gray-500 bg-gray-100'}`}>
                  {PUBLICATION_STATUS_LABELS[detail.publication.status]?.label ?? detail.publication.status}
                </span>
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Auteur : <span className="font-medium text-gray-700 dark:text-gray-300">{detail.publication.author.name}</span>
              </p>
...
export default function ReportedPublicationsPage() {
  ...
  const handleExport = (format: 'csv' | 'json' | 'pdf') => {
    ...
    if (format === 'json') {
      ...
      filename = 'signalements-publications.json';
    } else {
      ...
      filename = 'signalements-publications.csv';
    }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Publications signalés</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{data?.total ?? 0} publication(s) signalé(s)</p>
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-48">
            ...
            <input
              ...
              placeholder="Rechercher un publication ou auteur…"
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={9} className="px-6 py-12 text-center">
                      ...
                      <p className="text-sm text-gray-400">Aucun publication signalé trouvé</p>
                      {(riskFilter !== 'all' || statusFilter !== 'all' || search) && (
                  ...
                  data.items.map((item) => {
                    ...
                    const sc = PUBLICATION_STATUS_LABELS[item.publicationStatus];
                    return (
                      <tr
                        ...
                        onClick={() => setSelectedId(item.publicationId)}
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${sc?.cls ?? 'text-gray-500 bg-gray-100'}`}>
                            {sc?.label ?? item.publicationStatus}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3 text-right">
                          <button
                            onClick={(e) => { e.stopPropagation(); setSelectedId(item.publicationId); }}
