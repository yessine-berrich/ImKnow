...
interface DraftPublicationsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onDraftUpdated?: () => void;
}

export default function DraftPublicationsModal({
  isOpen,
  onClose,
  onDraftUpdated,
  ...
  const loadDrafts = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/publications/my/drafts`, {
  ...
  const handleDeleteDraft = async (e: React.MouseEvent, draftId: number) => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_URL}/publications/${draftId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3 px-6">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center">
                  Vos publications sauvegardés comme brouillons apparaîtront ici.
                </p>
              </div>
            ) : (
      ...
      {/* Reuse CreatePublicationModal in edit mode */}
