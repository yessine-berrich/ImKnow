// components/article/ShareArticleModal.tsx
'use client';
...
export default function ShareArticleModal({ isOpen, onClose, article }: ShareArticleModalProps) {
  ...
  const articleUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/home?article=${article.id}`
  ...
  const externalLinks = [
    ...
    },
    {
      ...
      url: `mailto:?subject=${encodedTitle}&body=${encodeURIComponent(`${article.title}\n\n${article.description ?? ''}\n\n${articleUrl}`)}`,
    },
    {
      label: 'X / Twitter',
    ...
    },
    {
      ...
      url: `sms:?&body=${encodeURIComponent(`${article.title}\n${articleUrl}`)}`,
    },
  ];
      ...
      <div className="relative w-full sm:max-w-md bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div className="flex-1 min-w-0 pr-3">
            <h2 className="text-base font-bold text-gray-900 dark:text-white">Partager l'article</h2>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 truncate">{article.title}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="px-5 pt-4 pb-3">
            <p className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">Lien de l'article</p>
            <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2">
              <span className="flex-1 text-xs text-gray-500 dark:text-gray-400 truncate font-mono">{articleUrl}</span>
              <button
                onClick={handleCopy}
