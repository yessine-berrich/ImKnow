// components/article/ReportArticleModal.tsx
'use client';
...
export default function ReportArticleModal({
  ...
  const handleSubmit = async () => {
    ...
    setErrorMessage('');
    try {
      await articleService.reportArticle(
        parseInt(articleId),
        selectedReason,
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <h2 className="text-base font-bold text-gray-900 dark:text-white">Signaler l'article</h2>
              <p className="text-xs text-gray-400 dark:text-gray-500 truncate max-w-[260px]">{articleTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
