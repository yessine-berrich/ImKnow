...
export class NotificationService {
  ...
  async createAndNotify(
    ...
    if (sendEmail) {
      try {
        ...
        // Continue anyway - email failure shouldn't block notification creation
      }
    }
    return saved;
  }
   ...
   * @returns true si l'email doit être envoyé, false sinon
   */
  async shouldSendEmail(recipientId: number, type: NotificationType): Promise<boolean> {
    ...
    // Si l'utilisateur n'existe pas ou a désactivé les emails globalement
